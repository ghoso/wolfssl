...
static void test_wolfSSL_PKCS8_Compat(void)
{
    ...
    AssertTrue(f != XBADFILE);
    AssertIntGT((bytes = (int)XFREAD(pkcs_buffer, 1, sizeof(pkcs_buffer), f)), 0);
    XFCLOSE(f);
    AssertNotNull(bio = BIO_new_mem_buf((void*)pkcs_buffer, bytes));
...
static void test_wolfSSL_BIO_gets(void)
{
    ...
    AssertNotNull(bio = BIO_new(BIO_s_mem()));
    AssertIntEQ(BIO_gets(bio, bio_buffer, 2), -1); /* nothing to read */
